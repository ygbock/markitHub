...
export class OrderLifecycleService {
  ...
  static initializeOrderLifecycle(order: Order, startStageId: number = 6): {
    ...
    const hasExplicitPaymentStatus = normalizedPaymentStatus.length > 0;
    const isPaid = ['paid', 'completed', 'settled', 'captured'].includes(normalizedPaymentStatus);
    const isPaymentPending = ['pending', 'pending_payment', 'pending verification', 'processing', 'requires_payment_method', 'requires_action'].includes(normalizedPaymentStatus)
      || ['pending payment', 'pending_payment', 'submitted'].includes(normalizedOrderStatus);

    let orderStatus: OrderDomainOrderStatus = 'Submitted';
    let paymentStatus: OrderDomainPaymentStatus = 'Pending Verification';
    let fulfillmentStatus: OrderDomainFulfillmentStatus = 'Reserved';
    ...
    } else if (normalizedPaymentStatus === 'authorized') {
      paymentStatus = 'Authorized';
    } else if (normalizedPaymentStatus === 'partially paid' || normalizedPaymentStatus === 'partially_paid') {
      paymentStatus = 'Partially Paid';
    } else if (normalizedPaymentStatus === 'failed' || normalizedPaymentStatus === 'payment_failed') {
      paymentStatus = 'Failed';
    } else if (normalizedPaymentStatus === 'refunded') {
      paymentStatus = 'Refunded';
    } else if (normalizedPaymentStatus === 'partially refunded' || normalizedPaymentStatus === 'partially_refunded') {
