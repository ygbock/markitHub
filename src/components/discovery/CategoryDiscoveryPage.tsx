        ...
        {!indexLoading && !indexError && filteredIndexCategories.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rootCategories.map(category => {
              const children = subcategoryMap.get(category.id) || [];
              return (
                <div
                  key={category.id}
                  className="bg-white rounded-3xl border border-slate-200 p-6 flex flex-col justify-between hover:border-indigo-300 hover:shadow-lg transition-all"
                >
                  <div>
                      ...
                      <div className="space-y-1.5 mb-4 pt-3 border-t border-slate-100">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {children.map(child => (
                            <button
                              key={child.id}
                              onClick={() => onNavigate(`/category/${child.slug}`)}
                              className="px-2.5 py-1 rounded-lg bg-slate-50 hover:bg-indigo-50 hover:text-indigo-700 text-xs font-semibold text-slate-600 transition-colors"
                            >
                              {child.name}
                            </button>
                          ))}
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-indigo-600 hover:text-white text-slate-800 text-xs font-black flex items-center justify-center gap-1.5 transition-all"
                  >
